...
options:
  ...
  --server              Server mode: launch the app under rocprofv3
                        and collect traces until Ctrl-C. Use for
                        long-running apps (e.g. LLM servers). Implies
                        --timeline-only. Combine with --kernel to
                        activate capture on first matching dispatch.
  --list-kernels        Run a trace pass and print unique kernel
                        names sorted by total GPU time, then exit.
                        Use this to discover kernel names for
                        --kernel filtering. Implies --timeline-only.
```
