...
def generate_report(command, gpu_specs, kernel_events, hip_events,
                    ...
                    compute_by_kernel=None, occupancy_by_kernel=None,
                    timestamp=None):
...
(function() {
  ...
  svg.querySelectorAll('.tl-ev:not(.gpu-ev)').forEach(function(el) {
    ...
    parts.append('<table><thead><tr><th>#</th><th>Kernel</th><th>Calls</th>'
                 '<th>Total</th><th>Avg</th><th>%</th><th>Occupancy</th></tr></thead><tbody>')
    for i, k in enumerate(top_kernels):
        color = KERNEL_COLORS[i % len(KERNEL_COLORS)]
        kname = _esc(_short_name(k["kernel_name"], 40))
        occ = (occupancy_by_kernel or {}).get(k["kernel_name"], {})
        occ_str = f'{occ["occupancy_pct"]:.1f}%' if occ else "—"
        parts.append(f'<tr>'
    ...
    for i, k in enumerate(top_kernels):
        ...
        # Occupancy stats
        occ = (occupancy_by_kernel or {}).get(kname)
        if occ:
            parts.append('<h3>Occupancy</h3>')
            parts.append('<div class="stats-grid">')
            parts.append(f'<div class="stat"><div class="label">Workgroup Size</div>'
                         f'<div class="value">{occ["wg_dims"]} ({occ["wg_size"]})</div></div>')
            parts.append(f'<div class="stat"><div class="label">Waves / WG</div>'
                         f'<div class="value">{occ["waves_per_wg"]}</div></div>')
            parts.append(f'<div class="stat"><div class="label">Occupancy</div>'
                         f'<div class="value">{occ["occupancy_pct"]:.1f}%</div></div>')
            parts.append('</div>')
...
def generate_markdown_report(command, gpu_specs, kernel_events, hip_events,
                             top_kernels, insts_by_kernel, roofline_by_kernel,
                             compute_by_kernel=None, occupancy_by_kernel=None,
                             timestamp=None):
    ...
    lines = []
    lines.append("# ROCm Profile Report")
    lines.append("")
    lines.append(f"- **Command:** `{command}`")
    lines.append(f"- **GPU:** {gpu_name} ({gfx}) — {gpu_specs.get('cu_count', '?')} CUs @ {gpu_specs.get('max_clock_mhz', '?')} MHz")
    ...
    lines.append("")
    lines.append("| # | Kernel | Calls | Total | Avg | % | Occupancy |")
    lines.append("|---|--------|-------|-------|-----|---|-----------|")
    for i, k in enumerate(top_kernels):
        kname = _short_name(k["kernel_name"], 50)
        occ = (occupancy_by_kernel or {}).get(k["kernel_name"], {})
        occ_str = f'{occ["occupancy_pct"]:.1f}%' if occ else "—"
        lines.append(f"| {i+1} | `{kname}` | {k['count']} | {_fmt_ns(k['total_ns'])} | {_fmt_ns(k['avg_ns'])} | {k['pct']:.1f}% | {occ_str} |")
    lines.append("")
    ...
    insts_order = [
        ...
        ("SQ_INSTS_SMEM", "SMEM"), ("SQ_INSTS_VMEM_RD", "VMEM_RD"),
        ("SQ_INSTS_VMEM_WR", "VMEM_WR"), ("SQ_INSTS_LDS", "LDS"),
        ("SQ_INSTS_FLAT", "FLAT"), ("SQ_INSTS_MFMA", "MFMA"),
    ]

    for i, k in enumerate(top_kernels):
        kname = k["kernel_name"]
        lines.append(f"### #{i+1}: {_short_name(kname, 60)}")
        lines.append("")
        lines.append(f"`{kname}`")
        lines.append("")

        # Duration stats
        lines.append(f"- **Total:** {_fmt_ns(k['total_ns'])} | **Avg:** {_fmt_ns(k['avg_ns'])} | **Min:** {_fmt_ns(k['min_ns'])} | **Max:** {_fmt_ns(k['max_ns'])}")
        lines.append(f"- **Calls:** {k['count']} | **% GPU Time:** {k['pct']:.1f}%")
        lines.append("")
        ...
        if occ:
            ...
            lines.append("")
            lines.append(f"- **Workgroup Size:** {occ['wg_dims']} ({occ['wg_size']} threads)")
            lines.append(f"- **Waves / WG:** {occ['waves_per_wg']}")
            lines.append(f"- **Occupancy:** {occ['occupancy_pct']:.1f}%")
            lines.append("")
        ...
        if total_insts > 0:
            ...
            lines.append("")
            lines.append("| " + " | ".join(label for _, label in insts_order) + " |")
            lines.append("| " + " | ".join("---:" for _ in insts_order) + " |")
            lines.append("| " + " | ".join(f"{int(insts.get(key, 0)):,}" for key, _ in insts_order) + " |")
            lines.append("")
        ...
        if roof:
            ...
            lines.append("")
            lines.append("| Level | Achieved | Peak | Utilization |")
            lines.append("|-------|----------|------|-------------|")
            for level, pct_key, achieved_key, peak_key in [
                ("HBM", "hbm_pct", "hbm_achieved_gbs", "hbm_bw_gbs"),
                ("L2",  "l2_pct",  "l2_achieved_gbs",  "l2_bw_gbs"),
                ("L1",  "l1_pct",  "l1_achieved_gbs",  "l1_bw_gbs"),
                ("LDS", "lds_pct", "lds_achieved_gbs", "lds_bw_gbs"),
            ]:
                lines.append(f"| {level} | {_fmt_gbs(roof[achieved_key])} | {_fmt_gbs(gpu_specs[peak_key])} | {roof[pct_key]:.1f}% |")
            lines.append("")
        ...
        if comp:
            ...
            lines.append("")
            lines.append("| Unit | Achieved | Peak | Utilization |")
            lines.append("|------|----------|------|-------------|")
            lines.append(f"| VALU (FP32) | {_fmt_tflops(comp['valu_tflops'])} | {_fmt_tflops(gpu_specs.get('peak_valu_tflops', 0))} | {comp['valu_pct']:.1f}% |")
            lines.append(f"| MFMA (FP16) | {_fmt_tflops(comp['mfma_tflops'])} | {_fmt_tflops(gpu_specs.get('peak_mfma_f16_tflops', 0))} | {comp['mfma_pct']:.1f}% |")
            lines.append(f"| SALU (INT) | {_fmt_gops(comp['salu_gops'])} | {_fmt_gops(gpu_specs.get('peak_salu_gops', 0))} | {comp['salu_pct']:.1f}% |")
            lines.append("")

        lines.append("---")
        lines.append("")

    lines.append("*Generated by rocm-profile-agent using rocprofv3*")
    return "\n".join(lines)
